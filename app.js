...
// Nueva ruta para clasificar clientes por compras
app.get('/analizar-clientes', (req, res) => {
  ...
  fs.readFile(filePath, 'utf8', (err, data) => {
    ...
    // Extraer el valor de compras o total
    const datos = clientes.map(c => [parseFloat(c.compras || c.total || 0) || 0]);

    // Clustering en 3 grupos
    const resultado = kmeans(datos, 3);

    const clasificados = clientes.map((cliente, i) => {
      return {
        ...cliente,
        grupo: `Grupo ${resultado.clusters[i] + 1}`
      };
    });
